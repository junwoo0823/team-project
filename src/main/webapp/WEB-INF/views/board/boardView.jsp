							...
							<c:if test="${ not empty sessionScope.id }">
								<c:if test="${ sessionScope.id eq board.memberId }">
									<button type="button" class="btn btn-outline-success"
										onclick="location='/board/modify?num=${ board.num }&pageNum=${ pageNum }'">
								...
								<button type="button" class="btn btn-outline-success"
									onclick="location='/board/reply?reRef=${ board.reRef }&reLev=${ board.reLev }&reSeq=${ board.reSeq }&pageNum=${ pageNum }'">
									<i class="bi bi-reply"></i>답글
								</button>
							</c:if>
							<button type="button" class="btn btn-outline-info"
								onclick="location='/board/list?pageNum=${ pageNum }'">
								<i class="bi bi-list-task"></i>글목록
							</button>
												...
												<c:forEach items="${ attachList }" var="attach">
													...
													<c:if test="${ attach.filetype eq 'I' }">
														...
														<c:set var="fileCallPathOrigin"
															value="${ attach.uploadpath }/${ attach.uuid }_${ attach.filename }" />
														<li><a
															href="/display?fileName=${ fileCallPathOrigin }"> <img
								...
								<table class="table-borderless table-sm">
									...
									<tr>
										<c:if test="${ not empty sessionScope.id }">
										   ...
										   <%-- <th><button class="btn btn-outline-success btn-sm" onclick="location='/comment/commentModify?num=${ comment.num }&pageNum=${ pageNum }'"> --%>
										   <!-- <i class="bi bi-pencil"></i>수정</button></th> -->
												
												<td><button class="btn btn btn-outline-danger btn-sm" onclick="location='/comment/commentRemove?num=${ comment.num }&pageNum=${ pageNum }'">
												<i class="bi bi-trash2"></i>삭제</button></td>
